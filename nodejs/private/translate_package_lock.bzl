...
load("@rules_nodejs//nodejs/private:js_library.bzl", "js_library")

js_library(
    name = "{name}",
    # TODO: not all npm packages are published with "package" as the top-level directory in the archive
    # some are self-named instead, so we'll have to run a repository_ctx.execute to read the disk
    # and find this name.
    srcs = glob(["package/**"]),
...
def _translate_package_lock(repository_ctx):
    build_content = [
        ...
        "",
        """load("@rules_nodejs//nodejs/private:npm_tarball.bzl", "npm_tarball")""",
        "",
    ]
    bzl_content = [
        "\"Generated by package_lock.bzl from %s\"" % str(repository_ctx.attr.package_lock),
        "",
        """load("@rules_nodejs//nodejs/private:download_and_extract.bzl", "bazel_download")""",
        "",
