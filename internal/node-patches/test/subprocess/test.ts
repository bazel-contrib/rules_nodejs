...
const requireScript = path.resolve(
...
function assertPatched(prefix: string, arr: string[]) {
  ...
  // this has a downside in that a user can delete it from execArgv and workerThreads will not use the loader.
  // in the future we may remove this loophole by making process.execArgv a getter or something.

  assert.deepStrictEqual(
    binDir,
    pathEnv.split(path.delimiter)[0],
    prefix +
      ' the highest priority directory in the PATH must be the node shim dir.'
  );
}
...
describe('spawning child processes', () => {
  ...
  it('get patched if run as shell script.', () => {
    const res = cp.execSync(
      `NP_SUBPROCESS_BIN_DIR=${tmp} node -r ${requireScript} -e 'console.log(JSON.stringify([process.execPath,process.execArgv,process.argv,process.env.PATH]))'`
    );
  ...
  it('overwrites spawn related variables correctly.', () => {
    const res = cp.execSync(
      `NP_SUBPROCESS_BIN_DIR=${tmp} node -r ${requireScript} ${path.join(
        __dirname,
        'worker-threads-script.js'
      )}`
    );
