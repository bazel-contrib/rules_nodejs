...
function doResolve(importee, importer) {
  return rollupConfig.resolveBazel(importee, importer, baseDir, resolve)
      .replace(/\\/g, '/');
}
...
describe("rollup config", () => {
  fit("should resolve relative imports", () => {
    expect(
        doResolve(`.${sep}a`, join(baseDir, "bazel-bin", "path", "to", "b.esm5",
                                   "some_wksp", "path", "to", "b")))
        .toEqual(
            `${baseDir}/bazel-bin/path/to/b.esm5/some_wksp/path/to/a/index.js`);
    expect(doResolve(`..${sep}a`,
                     join(baseDir, "bazel-bin", "path", "to", "b.esm5",
                          "some_wksp", "path", "to", "b", "sub")))
        .toEqual(
            `${baseDir}/bazel-bin/path/to/b.esm5/some_wksp/path/to/a/index.js`);
  });
  ...
  it("should find paths using module mapping", () => {
    expect(doResolve("foo/bar"))
        .toEqual(`${
            baseDir}/bazel-bin/path/to/a.esm5/my_workspace/path/to/foo_lib/bar`);
    expect(doResolve("other/thing"))
        .toEqual(`${
            baseDir}/bazel-bin/path/to/b.esm5/other_wksp/path/to/other_lib/thing`);
  });
