...
`;
  ...
  let buildFile = generateBuildFileHeader() + `load("@rules_nodejs//:index.bzl", "js_library")
...
load("@rules_nodejs//internal/copy_repository:copy_repository.bzl", "copy_repository")
...
function printPackage(pkg: Dep) {
  ...
  let result = `load("@rules_nodejs//:index.bzl", "js_library")
...
`;
  ...
  if (mainEntryPoint && !findFile(pkg, `${pkg._name}.umd.js`)) {
    result += `load("@rules_nodejs//internal/npm_install:npm_umd_bundle.bzl", "npm_umd_bundle")
...
export function printPackageBin(pkg: Dep) {
  ...
  if (executables.size) {
    result = `load("@rules_nodejs//:index.bzl", "nodejs_binary")
...
export function printIndexBzl(pkg: Dep) {
  ...
  const executables = _findExecutables(pkg);
  if (executables.size) {
    result = `load("@rules_nodejs//:index.bzl", "nodejs_binary", "nodejs_test", "npm_package_bin")
...
function printScope(scope: string, pkgs: Dep[]) {
  ...
  return `load("@rules_nodejs//:index.bzl", "js_library")
