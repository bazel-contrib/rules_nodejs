...
if (DEBUG)
  console.error(`
node_patcher: running ${TARGET} with
  cwd: ${process.cwd()}
...
function loadRunfilesManifest(manifestPath) {
  if (DEBUG) console.error(`node_patcher: using manifest ${manifestPath}`);
  ...
  if (DEBUG) console.error(`node_patcher: using binRoot ${binRoot}`);
  if (DEBUG) console.error(`node_patcher: using genRoot ${genRoot}`);
  if (DEBUG) console.error(`node_patcher: using localWorkspacePath ${localWorkspacePath}`);
...
function resolveRunfiles(parent, ...pathSegments) {
  ...
  if (runfilesManifest) {
    ...
    if (DEBUG) console.error('node_patcher: try to resolve in runfiles manifest', runfilesEntry);

    let maybe = resolveManifestFile(runfilesEntry);
    if (maybe) {
      if (DEBUG) console.error('node_patcher: resolved manifest file', maybe);
      return maybe;
    }

    maybe = resolveManifestDirectory(runfilesEntry);
    if (maybe) {
      if (DEBUG) console.error('node_patcher: resolved via manifest directory', maybe);
      return maybe;
    }
  } else {
    if (DEBUG) console.error('node_patcher: try to resolve in runfiles', runfilesPath);

    let maybe = loadAsFileSync(runfilesPath);
    if (maybe) {
      if (DEBUG) console.error('node_patcher: resolved file', maybe);
      return maybe;
    }

    maybe = loadAsDirectorySync(runfilesPath);
    if (maybe) {
      if (DEBUG) console.error('node_patcher: resolved via directory', maybe);
      return maybe;
    }
  }
...
module.constructor._resolveFilename = function(request, parent, isMain, options) {
  ...
  try {
    ...
    if (resolved === request || request.startsWith('.') || request.startsWith('/') ||
      ...
      if (DEBUG)
        console.error(
            `node_patcher: resolved ${request} to built-in, relative or absolute import ` +
      ...
      if (parentNodeModulesSegment != -1) {
        ...
        if (!relative.startsWith('..')) {
          ...
          if (DEBUG)
            console.error(
                `node_patcher: resolved ${request} within parent node_modules to ` +
                `${resolved} from ${parentFilename}`);
          return resolved;
        } else {
          throw new Error(
  ...
  try {
    ...
    if (DEBUG)
      console.error(`node_patcher: resolved ${request} within runfiles to ${resolved} from ${
          parentFilename}`);
    return resolved;
  } catch (e) {
  ...
  if (relativeParentFilename && !relativeParentFilename.startsWith('..')) {
    ...
    if (parentSegments[0] !== USER_WORKSPACE_NAME) {
      try {
        ...
        if (DEBUG)
          console.error(
              `node_patcher: resolved ${request} within node_modules ` +
  ...
  try {
    ...
    if (DEBUG)
      console.error(
          `node_patcher: resolved ${request} within node_modules (${NODE_MODULES_ROOT}) to ` +
          `${resolved} from ${parentFilename}`);
    return resolved;
  } catch (e) {
