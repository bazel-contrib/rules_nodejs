...
NODE_URLS = ["https://nodejs.org/dist/v{}/{}"]
YARN_URLS = ["https://github.com/yarnpkg/yarn/releases/download/v{}/{}"]
...
def _download_node(repository_ctx):
  ...
  repository_ctx.download_and_extract(
    url = [url.format(node_version, filename) for url in node_urls],
    output = "node",
...
def _download_yarn(repository_ctx):
  ...
  repository_ctx.download_and_extract(
    url = [url.format(yarn_version, filename) for url in yarn_urls],
    output = "yarn",
    stripPrefix = strip_prefix,
    sha256 = sha256,
  )
...
def _prepare_node(repository_ctx):
  ...
  node_exec = "{}/bin/node".format(node_path) if host.find("windows") == -1 else  "{}/node.exe".format(node_path)
  npm_script = "{}/bin/npm".format(node_path) if host.find("windows") == -1 else "{}/node_modules/npm/bin/npm-cli.js".format(node_path)
  yarn_script = "{}/bin/yarn.js".format(yarn_path)
  node_entry = "repository_bin/node" if host.find("windows") == -1 else "repository_bin/node.cmd"
  npm_entry = "repository_bin/npm" if host.find("windows") == -1 else "repository_bin/npm.cmd"
  yarn_entry = "repository_bin/yarn" if host.find("windows") == -1 else "repository_bin/yarn.cmd"
...
alias(name = "node", actual = "{}")
alias(name = "npm", actual = "{}")
alias(name = "yarn", actual = "{}")
""".format(node_entry, npm_entry, yarn_entry))
...
spawnSync("{}", process.argv.slice(2), {{stdio: [process.stdin, process.stdout, process.stderr]}});
""".format(
    repository_ctx.path("repository_bin"),
    node_exec))
...
""".format(
    repository_ctx.path("repository_bin"),
    node_exec,
    repository_ctx.path("repository_bin/node.js")))
  else:
...
""" + "".join(["""
(cd "{}"; "{}" "{}" --scripts-prepend-node-path=false "$@")
""".format(
    repository_ctx.path(package_json).dirname,
    repository_ctx.path(node_entry),
    npm_script)
...
""" + "".join(["""
cd "{}" && call "{}" "{}" --scripts-prepend-node-path=false %*
""".format(
    repository_ctx.path(package_json).dirname,
    repository_ctx.path(node_entry),
    npm_script)
...
package(default_visibility = ["//visibility:public"])
alias(name = "yarn", actual = "{}")
""".format(get_yarn_label(repository_ctx)))
