...
exports_files(["{0}", "run_npm.sh.template", "args.sh"])
...
""".format(repository_ctx.path(node), repository_ctx.path(npm)))
  ...
  repository_ctx.file("args.sh", content="""#!/bin/bash
...
def node_repositories(package_json, node_version=DEFAULT_NODE_VERSION, yarn_version=DEFAULT_YARN_VERSION, preserve_symlinks=False):
