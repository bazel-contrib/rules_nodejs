...
def _prepare_node(repository_ctx):
  ...
  shebang = "/usr/bin/env bash" if not is_windows else "/bin/bash"
...
""".format(
  ...
  repository_ctx.file("bin/node_args.sh", content="""#!{shebang}
#Generated by node_repositories.bzl
export NODE_REPOSITORY_ARGS={args}
""".format(
    args = "--node_options=--preserve-symlinks" if repository_ctx.attr.preserve_symlinks else "",
    shebang = shebang), executable = True)
  ...
  if not is_windows:
    # Npm entry point
    repository_ctx.file("bin/npm", content="""#!{shebang}
#Generated by node_repositories.bzl
...
""".format(
    ...
    repository_ctx.file("bin/npm_node_repositories", content="""#!{shebang}
#Generated by node_repositories.bzl
...
""".format(
  ...
  if not is_windows:
    # Yarn entry point
    repository_ctx.file("bin/yarn", content="""#!{shebang}
#Generated by node_repositories.bzl
"{node}" "{script}" "$@"
""".format(
    node = repository_ctx.path(node_entry),
