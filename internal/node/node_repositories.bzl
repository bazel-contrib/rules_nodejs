...
def _prepare_node(repository_ctx):
  ...
  npm_node_repositories_entry = "repository_bin/npm_node_repositories" if not is_windows else "repository_bin/npm_node_repositories.cmd"
  yarn_node_repositories_entry = "repository_bin/yarn_node_repositories" if not is_windows else "repository_bin/yarn_node_repositories.cmd"
...
""".format(
    ...
    repository_ctx.file("repository_bin/npm_node_repositories", content="""#!/bin/bash
#Generated by node_repositories.bzl
...
""".format(
    ...
    executable = True)
    # Npm entry point for node_repositories
    repository_ctx.file("repository_bin/npm_node_repositories.cmd", content="""@echo off
""" + "".join(["""
...
""".format(
    ...
    repository_ctx.file("repository_bin/yarn_node_repositories", content="""#!/bin/bash
#Generated by node_repositories.bzl
#Executes the given NPM command over each of the package.json folders provided in node_respositories.
""" + "".join(["""
...
""".format(
    ...
    executable = True)
    # Yarn entry point for node_repositories
    repository_ctx.file("repository_bin/yarn_node_repositories.cmd", content="""@echo off
""" + "".join(["""
