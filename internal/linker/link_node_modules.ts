...
const {runfiles: _defaultRunfiles, _BAZEL_OUT_REGEX}: typeof import('@bazel/runfiles') = require('../runfiles/index.js');
...
export async function main(args: string[], runfiles: Runfiles) {
  ...
  let {workspace, bin, module_sets} = JSON.parse(fs.readFileSync(modulesManifest));
  log_verbose('manifest contents:', JSON.stringify({workspace, bin, module_sets}, null, 2));
  ...
  async function linkModules(package_path: string, m: LinkerTreeElement) {
    ...
    if (path.dirname(m.name)) {
      ...
      if (package_path) {
        await mkdirp(`${bin}/${symlinkIn}/${path.dirname(m.name)}`);
      }
    }
    ...
    if (m.link) {
      ...
      if (!isExecroot || !existsSync(target)) {
        ...
        try {
          ...
          if (runfiles.manifest && modulePath.startsWith(`${bin}/`)) {
            ...
            if (!target || !target.match(_BAZEL_OUT_REGEX)) {
      ...
      const symlinkBinFile = package_path ? `${bin}/${symlinkIn}/${m.name}` : undefined;
      const statsBin = symlinkBinFile ? await gracefulLstat(symlinkBinFile) : null;
      const isLeftOverBin =
          (symlinkBinFile && statsBin !== null && await isLeftoverDirectoryFromLinker(statsBin, symlinkBinFile));
      ...
      if (target && await exists(target)) {
        ...
        if (symlinkBinFile) {
          if (statsBin !== null && isLeftOverBin) {
            await createSymlinkAndPreserveContents(statsBin, symlinkBinFile, target);
          } else {
            await symlinkWithUnlink(target, symlinkBinFile, statsBin);
          }
        }
      } else {
        if (!target) {
        ...
        if (symlinkBinFile && isLeftOverBin) {
          // Remove left over directory if it exists
          await unlink(symlinkBinFile);
        }
      }
    }
