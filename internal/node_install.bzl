...
def _node_impl(repository_ctx):
  ...
  executables = get_executables(repository_ctx)
...
""".format(executables["node"]))
  repository_ctx.file("npm.sh", content="""#!/bin/bash
#Generated by node_install.bzl
""" + "".join(["""
ROOT="{}"
NODE="{}"
SCRIPT="{}"
...
""".format(
    ...
    repository_ctx.path(executables["node"]),
    repository_ctx.path(executables["npm"]))
