...
readonly LIBRARY_JS=$(cat $(rlocation "build_bazel_rules_nodejs/packages/typescript/test/some_library/library.js"))
readonly BAR_JS=$(cat $(rlocation "build_bazel_rules_nodejs/packages/typescript/test/foobar/bar.js"))
readonly FOO_JS=$(cat $(rlocation "build_bazel_rules_nodejs/packages/typescript/test/foobar/foo.js"))
...
# should produce named UMD modules
if [[ "$BAR_JS" != *"define(\"build_bazel_rules_nodejs/packages/typescript/test/foobar/bar\""* ]]; then
...
if [[ "$BAR_JS" != *"require(\"build_bazel_rules_nodejs/packages/typescript/test/foobar/foo\")"* ]]; then
...
if [[ "$BAR_JS" != *"require(\"build_bazel_rules_nodejs/packages/typescript/test/generated_ts/foo\")"* ]]; then
...
# should produce named UMD modules
if [[ "$FOO_JS" != *"define(\"build_bazel_rules_nodejs/packages/typescript/test/foobar/foo\""* ]]; then
