...
function createWatchProgram(options, tsconfigPath, setTimeout) {
  const host = createWatchCompilerHost(
    tsconfigPath,
    options,
    Object.assign(Object.assign({}, ts.sys), { setTimeout }),
...
function getWatchProgram(args) {
  ...
  if (!cachedWatchedProgram) {
    ...
    cachedWatchProgramArgs = newWatchArgs;
    cachedWatchedProgram = createWatchProgram(
      parsedArgs.options,
      tsconfigPath,
      (callback) => {
...
function emitOnce(args) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const result = yield watchProgram === null || watchProgram === void 0
      ? void 0
      : watchProgram.getProgram().emit(undefined, undefined, {
...
function main() {
  const typescriptRequirePath =
    process.argv[process.argv.indexOf("--typescript_require_path") + 1];
  try {
    ...
    worker.log(
      `typescript_require_path '${typescriptRequirePath}' could not be resolved`
    );
    throw e;
  }
  if (process.argv.includes("--persistent_worker")) {
    ...
    emitOnce(args).finally(() =>
      cachedWatchedProgram === null || cachedWatchedProgram === void 0
        ? void 0
        : cachedWatchedProgram.close()
    );
  }
