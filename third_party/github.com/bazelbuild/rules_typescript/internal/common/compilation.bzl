...
COMMON_ATTRIBUTES = {
    ...
    "use_precise_ts_files": attr.bool(
        default = True,
    ),
    "ambient_srcs": attr.label_list(
        doc = "The TypeScript source files to compile. These will also be added as inputs to be eagerly resolved by reverse deps.",
        allow_files = [".ts"],
    ),
...
def _collect_dep_declarations(ctx, declaration_infos):
    ...
    # .d.ts files from direct ambient dependencies, ok for strict deps
    ambient_deps_declarations = [dep.ambient_declarations for dep in declaration_infos if getattr(dep, "ambient_declarations", None)]

    # .d.ts files from transitive ambient dependencies
    transitive_ambient_deps_declarations = [dep.transitive_ambient_declarations for dep in declaration_infos if getattr(dep, "transitive_ambient_declarations", None)]
    ...
    if hasattr(ctx.attr, "node_modules"):
        if DeclarationInfo in ctx.attr.node_modules:
            ...
            if getattr(ctx.attr.node_modules[DeclarationInfo], "transitive_ambient_declarations", None):
                transitive_ambient_deps_declarations.append(ctx.attr.node_modules[DeclarationInfo].transitive_ambient_declarations)
            ...
            if getattr(ctx.attr.node_modules.typescript, "transitive_ambient_declarations", None):
                transitive_ambient_deps_declarations.append(ctx.attr.node_modules.typescript.transitive_ambient_declarations)
    ...
    return DeclarationInfo(
        declarations = depset(transitive = direct_deps_declarations + ambient_deps_declarations),
        transitive_declarations = depset(ctx.files._additional_d_ts, transitive = transitive_deps_declarations + transitive_ambient_deps_declarations),
        ambient_declarations = depset(transitive = ambient_deps_declarations),
        transitive_ambient_declarations = depset(transitive = transitive_ambient_deps_declarations),
...
def compile_ts(
    ...
    ambient_srcs = getattr(ctx.attr, "ambient_srcs", [])
    ...
    all_srcs = srcs + ambient_srcs
    tsconfig = tsconfig if tsconfig != None else ctx.outputs.tsconfig
    srcs_files = [f for t in srcs for f in t.files.to_list()]
    ambient_srcs_files = [f for t in ambient_srcs for f in t.files.to_list()]
    all_srcs_files = srcs_files + ambient_srcs_files
    src_declarations = []  # d.ts found in srcs and ambient_srcs.
    src_ambient_declarations = [] # .d.ts found in ambient_srcs.
    tsickle_externs = []  # externs.js generated by tsickle, if any.
    has_sources = False

    for src in all_srcs:
        is_ambient_src = src in ambient_srcs
        ...
        for f in src.files.to_list():
            ...
            if f.path.endswith(".d.ts"):
                src_declarations += [f]
                if is_ambient_src:
                    src_ambient_declarations += [f]
                continue


    outs = outputs(ctx, ctx.label, all_srcs_files)
    transpiled_closure_js = outs.closure_js
    transpiled_devmode_js = outs.devmode_js
    gen_declarations = outs.declarations
    gen_ambient_declarations = outputs(ctx, ctx.label, ambient_srcs_files).declarations
    ...
    if not is_library and not _should_generate_externs(ctx):
        type_blacklisted_declarations = depset(all_srcs_files, transitive = [type_blacklisted_declarations])
    ...
    compilation_bazel_inputs = dep_declarations.transitive_declarations.to_list() + dep_declarations.transitive_ambient_declarations.to_list() + all_srcs_files
    compilation_tsc_inputs = dep_declarations.transitive_ambient_declarations.to_list() + all_srcs_files if ctx.attr.use_precise_ts_files else compilation_bazel_inputs
    ...
    allowed_deps = depset(
        ...
        transitive = [dep_declarations.declarations, dep_declarations.ambient_declarations],
    )

    tsconfig_es6 = tsc_wrapped_tsconfig(
        ctx,
        compilation_tsc_inputs,
        all_srcs_files,
    ...
    if has_sources:
        inputs = compilation_bazel_inputs + [tsconfig] + getattr(ctx.files, "angular_assets", [])
        ...
        devmode_compile_action(
            ctx,
            compilation_bazel_inputs + [tsconfig_json_es5] + getattr(ctx.files, "angular_assets", []),
            outputs,
            tsconfig_json_es5,
            node_profile_args,
        )
    ...
    ambient_declarations_depsets = [depset(src_ambient_declarations + gen_ambient_declarations)]
    ...
    transitive_decls = depset(src_declarations + gen_declarations, transitive = [dep_declarations.transitive_declarations, dep_declarations.transitive_ambient_declarations] + ambient_declarations_depsets)

    declarations_provider = DeclarationInfo(
        declarations = depset(transitive = declarations_depsets + ambient_declarations_depsets),
        transitive_declarations = transitive_decls,
        ambient_declarations = depset(transitive = ambient_declarations_depsets),
        transitive_ambient_declarations = depset(transitive = ambient_declarations_depsets + [dep_declarations.transitive_ambient_declarations]),
    ...
    return {
        ...
        "instrumented_files": {
            ...
            "extensions": ["ts"],
            "source_attributes": ["srcs", "ambient_srcs"],
        },
